...
public class UserDao implements DaoInterface<User>{
    ...
    public List<User> findAll() {
        try {
           DbService.Result result = DbService.getInstance().execute("SELECT id, username, password, email, user_group_id FROM users");
           List<User> users =new ArrayList<>();
            result.getRows().forEach(row->{
               users.add(new User().setValues(row));
           });
           return users;
    ...
    public User findById(Long id) {
        try {
            DbService.Result result = DbService.getInstance().execute("SELECT id, username, password, email, user_group_id FROM users WHERE id=?",
                    String.valueOf(id));
